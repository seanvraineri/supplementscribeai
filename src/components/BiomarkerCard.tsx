        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center space-x-3">
            <span className={`px-4 py-2 rounded-xl text-sm font-semibold shadow-lg ${
              analysis.statusColor === 'green' ? 'bg-gradient-to-r from-emerald-500/20 to-green-500/20 text-emerald-400 border border-emerald-500/30' :
              analysis.statusColor === 'yellow' ? 'bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-400 border border-yellow-500/30' :
              analysis.statusColor === 'red' ? 'bg-gradient-to-r from-red-500/20 to-pink-500/20 text-red-400 border border-red-500/30' :
          ...
          <div className="space-y-2">
            ...
            {biomarker.reference_range && (
              <p className="text-sm text-dark-secondary bg-dark-border/30 rounded-lg px-3 py-1 inline-block">
                Lab Range: {biomarker.reference_range}
              </p>
            )}
          </div>
            ...
            <div className="flex items-start space-x-2">
              ...
              <div className="space-y-1">
                <p className="text-dark-primary leading-relaxed">
                  {analysis.inRangeStatus || analysis.interpretation || 'Data captured successfully'}
                </p>
                {analysis.referenceRange && (
                  <p className="text-xs text-dark-secondary">
                    Optimal: {analysis.referenceRange}
                  </p>
                )}
              </div>
            </div>
