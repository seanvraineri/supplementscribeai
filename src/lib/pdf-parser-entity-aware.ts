...
interface MedicalEntity {
  text: string;
  type: 'biomarker' | 'snp' | 'measurement' | 'condition' | 'medication' | 'procedure';
  context: string;
  confidence: number;
  position: {
    start: number;
...
export class EntityAwarePDFParser {
  // Medical entity patterns - flexible and comprehensive
  private medicalEntityPatterns = {
   ...
   */
  async parseText(text: string): Promise<{
   ...
   */
  private getContextWindow(text: string, position: number, currentLine: string): string {
   ...
   */
  private calculateEntityConfidence(entityText: string, type: string): number {
   ...
   */
  private extractBiomarkersFromEntities(entities: MedicalEntity[], fullText: string): ExtractedBiomarker[] {
   ...
   */
  private extractBiomarkerFromContext(entity: MedicalEntity, fullText: string): ExtractedBiomarker | null {
    ...
    if (valueMatch) {
      ...
      // Determine status
      let status: 'normal' | 'high' | 'low' | 'critical' | undefined;
   ...
   */
  private extractSNPsFromEntities(entities: MedicalEntity[], fullText: string): ExtractedSNP[] {
   ...
   */
  private extractSNPFromContext(entity: MedicalEntity): ExtractedSNP | null {
    const context = entity.context;
    const entityText = entity.text;
    
    let snpId: string | undefined;
    let geneName: string | undefined;
    let genotype: string = entityText;
   ...
   */
  private calculateConfidence(biomarkers: ExtractedBiomarker[], snps: ExtractedSNP[], entities: MedicalEntity[]): number {
